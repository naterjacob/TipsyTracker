...
function makeDb(routes: Array<[Handler, StmtResult]>) {
  ...
  const db = {
    prepare: vi.fn((query: string) => {
      const route = routes.find(([needle]) =>
...
describe("dal", () => {
  ...
  describe("listFeed", () => {
    ...
    it("maps rows and returns nextCursor when page is full (no cursor)", async () => {
      const { db, calls } = makeDb([
        ["FROM posts p", { all: [row] }]
      ]);
      const out = await listFeed(db, {
        limit: 1,
        cursor: null
      });
  ...
  describe("getPostDetails", () => {
    ...
    it("returns post with mapped stops", async () => {
      ...
      expect(out?.imageUrl).toBe("data:image/webp;base64,abc");
  ...
  describe("createDraftPost", () => {
    it("returns null when an existing draft exists", async () => {
      ...
      expect(
        await createDraftPost(db, { userId: "u1", now: 1 })
      ).toBeNull();
    });
    ...
    it("creates and returns a new draft", async () => {
      ...
      const out = await createDraftPost(db, {
        userId: "u1",
        now: 7
      });
  ...
  describe("updateDraftCaption", () => {
    it("returns null when not an owned draft", async () => {
      const { db } = makeDb([
        ["AND status = 'draft' LIMIT 1", { first: null }]
      ]);
      expect(
        await updateDraftCaption(db, {
          postId: "p1",
          userId: "u1",
          caption: "x"
        })
    ...
    it("updates and returns the post", async () => {
      ...
      const update = calls.find((c) =>
        c.query.includes("UPDATE posts SET caption")
      );
  ...
  describe("publishDraftPost", () => {
    ...
    it("returns no_stops when there are no stops", async () => {
      const { db } = makeDb([
        ...
        [
          "AS total_drinks",
          { first: { total_drinks: 0, stop_count: 0 } }
        ]
      ]);
      expect(
        (
    ...
    it("publishes and returns ok", async () => {
      const { db } = makeDb([
        ...
        [
          "AS total_drinks",
          { first: { total_drinks: 4, stop_count: 2 } }
        ],
  ...
  describe("deletePost", () => {
    it("returns false when not owner", async () => {
      const { db } = makeDb([
        ["SELECT id FROM posts", { first: null }]
      ]);
      expect(
        await deletePost(db, { postId: "p1", userId: "x" })
      ).toBe(false);
    });
    ...
    it("deletes children then post and returns true", async () => {
      ...
      expect(
        await deletePost(db, { postId: "p1", userId: "u1" })
      ).toBe(true);
      const deletes = calls
  ...
  describe("addDraftStop", () => {
    ...
    it("inserts a stop with incremented order", async () => {
      const { db, calls } = makeDb([
        ...
        [
          "SELECT id, post_id, bar_id",
          { first: { id: "s1", stop_order: 3 } }
        ]
      ]);
      ...
      const insert = calls.find((c) =>
        c.query.includes("INSERT INTO stops")
      );
  ...
  describe("getUserIdByUsername", () => {
    it("returns id (normalized, case-insensitive)", async () => {
      ...
      expect(await getUserIdByUsername(db, "  Tester ")).toBe(
        "u1"
      );
  ...
  describe("getUserProfile", () => {
    it("returns null when user missing", async () => {
      const { db } = makeDb([
        ["FROM users WHERE id = ?", { first: null }]
      ]);
    ...
    it("returns profile with stats and most-visited bar", async () => {
      const { db } = makeDb([
        ...
        [
          "AS visits",
          { first: { id: "b1", name: "Anchor", visits: 4 } }
        ]
      ]);
      const out = await getUserProfile(db, "u1");
      expect(out?.stats.totalDrinks).toBe(5);
      expect(out?.stats.mostVisitedBar).toMatchObject({
        name: "Anchor"
      });
    });
  ...
  describe("listUserPosts", () => {
    it("no cursor: returns posts and nextCursor when full", async () => {
            ...
            all: [
              {
                ...
                user_id: "u1",
                caption: "hi",
                image_url: "data:image/jpeg;base64,x",
                status: "published",
                total_drinks: 2,
                created_at: 1,
                published_at: 50
      ...
      const out = await listUserPosts(db, {
        userId: "u1",
        limit: 1,
        cursor: null
      });
  ...
  describe("createComment", () => {
    it("returns null when post not published", async () => {
      const { db } = makeDb([
        ["FROM posts WHERE id = ?", { first: null }]
      ]);
      expect(
    ...
    it("creates and returns the comment", async () => {
      ...
      const insert = calls.find((c) =>
        c.query.includes("INSERT INTO comments")
      );
  ...
  describe("unlikePost", () => {
    it("returns true when a like is removed", async () => {
      ...
      expect(
        await unlikePost(db, { postId: "p1", userId: "u1" })
      ).toBe(true);
    });
  ...
  describe("getPostLikeStatus", () => {
    it("returns count and likedByMe true", async () => {
      const { db } = makeDb([
        [
          "AS like_count",
          { first: { like_count: 3, liked_by_me: 1 } }
        ]
      ]);
      expect(
        await getPostLikeStatus(db, {
          postId: "p1",
          userId: "u1"
        })
